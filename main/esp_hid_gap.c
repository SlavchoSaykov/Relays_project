...
//        num_events_call++;
//        ESP_LOGI(TAG, "Number of ESP_GAP_BLE_KEY_EVT = %d", num_events_call);
...
#if CONFIG_BT_BLE_ENABLED
//    ESP_LOGE(TAG, "CONFIG_BT_BLE_ENABLED!");
    if (start_ble_scan(seconds) == ESP_OK) {
//        ESP_LOGE(TAG, "STEP : BLE SCAN");
        WAIT_BLE_CB();
    } else {
        return ESP_FAIL;
    }
...
#if CONFIG_BT_HID_HOST_ENABLED
//    ESP_LOGE(TAG, "CONFIG_BT_HID_HOST_ENABLED!");
...
//    ESP_LOGE(TAG, "\nnum_ble_scan_results = %d\nnum_bt_scan_results = %d", num_ble_scan_results, num_bt_scan_results);
